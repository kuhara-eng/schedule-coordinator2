...
function bindEvents() {
  ...
  els.fillAvailableBtn.addEventListener("click", () => {
    ...
    state.dates.forEach((date) => {
      times.forEach((time) => {
        setAvailability(selectedParticipantId, date, time, true);
      });
    });
    persist();
    renderInputGrid();
...
function renderCloudStatus(message) {
  ...
  els.copyShareBtn.disabled = !cloud.configured || !cloud.shareId;
}
...
async function initCloudSync() {
  ...
  renderCloudStatus();
  if (!cloud.configured || !cloud.shareId) return;
...
async function createShareLink() {
  ...
  setShareIdInUrl(cloud.shareId);
  renderCloudStatus("共有リンクを作成しています...");
  await saveSharedBoardNow();
  subscribeSharedBoard();
  renderCloudStatus();
}
...
async function loadSharedBoard() {
  ...
  const { data, error } = await cloud.client
    ...
    .select("data")
    .eq("share_id", cloud.shareId)
    .maybeSingle();

  if (error) {
    console.error(error);
    renderCloudStatus("共有データの読み込みに失敗しました。ローカル保存で動作中です。");
    return;
  }

  if (data?.data?.meetings?.length) {
    cloud.applyingRemote = true;
    appState = normalizeAppState(data.data);
    syncActiveMeeting();
...
function applyRemoteState(remoteState) {
  if (!remoteState?.meetings?.length) return;
...
function scheduleCloudSave() {
  if (!cloud.client || !cloud.shareId || cloud.applyingRemote) return;
...
function setShareIdInUrl(shareId) {
...
function deleteMeeting(meetingId) {
  ...
  if (appState.activeMeetingId === meetingId) {
    appState.activeMeetingId = appState.meetings[0].id;
  }
  syncActiveMeeting();
  selectedParticipantId = state.participants[0]?.id || null;
  persist();
